...
拦截请求---->>>>允许登录后访问(通过浏览器中保留的session信息)传统设置只能在jsp中设置，其他资源不能使用

通过Filter过滤实现根据权限过滤请求---同样需要配置Web.xml实现----设置拦截的资源路径，增加doFilter实现拦截逻辑 

**生命周期**

构造器----init初始化方法----doFilter实现拦截逻辑----destory销毁

**FilterConfig类**：Filter的配置信息

1. 获取Filter的名称
2. 获取Filter配置中的初始化参数
3. 获取ServletContext对象

Filter过滤器链---FilterChain----根据Filter在web.xml中配置的顺序来设置执行顺序，如果有下一个Filter会继续执行，如果没有会指向目标资源

所有Filter和目标资源默认执行在同一个线程中，多个Filter执行的时候使用的都是同一个Request对象（设置request域对象中的值后面也可以进行访问）

#### Filte拦截路径（设置url-pattern）设置拦截路径小技巧
...
两种存在形式---字符串/对象（使用给定API进行转换）---字符串一般用于传递数据
...
**创建一个类来得到返回的类型---匿名内部类实现**

List集合和JSON的相互转换

Map和JSON的相互转换

### AJAX请求--- js发起异步请求，局部更新页面

同步与异步：同步是指完完全全按照顺序执行，上一个没结束之前是不会执行下一个，而异步则是指同时执行，上一个还没结束也会执行下一个，异步不会影响用户体验
...
通过封装实现AJAX请求---$.get()  $.post()

**直接规定返回类型**

getJson封装$.get()获取的返回类型为JSON
...
- Locale--不同的时区，位置，语言
- Propertion 属性配置文件，配置不同地区的配置文件
- ResourceBundle对象获取对应的配置文件得到对应key的属性值 
